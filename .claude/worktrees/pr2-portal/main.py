...
# Secreto compartido con authservise/catalago (05/ago/2026) -- catalago
# empezo a exigirlo en POST /suscripciones (antes ese endpoint no tenia
# ninguna auth, cualquier proceso con red hacia catalago podia
# autoasignarse una suscripcion gratis). Sin este valor configurado,
# api_suscribirse recibe 401 de catalago -- fail-closed, no hay fallback:
# ver secret/coupertech-portal en Vault (mismo valor que
# INTERNAL_VINCULACION_SECRET en authservise/catalago).
INTERNAL_VINCULACION_SECRET = os.getenv("INTERNAL_VINCULACION_SECRET", "")
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            sub_resp = await client.post(
                ...
                headers={"X-Internal-Secret": INTERNAL_VINCULACION_SECRET},
            )
    except httpx.HTTPError:
